...
For a ChatGPT developer test, enable developer mode, install Attenza from the local repository marketplace, and start a new conversation with the plugin enabled. The registered app mapping supplies the same `https://www.attenza.io/mcp` connection, while the bundled skill supplies the polling, expiry, and A2UI decision workflow.
