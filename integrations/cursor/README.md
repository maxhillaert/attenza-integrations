...
After that link, `~/.cursor/plugins/local/attenza/plugin.json`, `mcp.json`, and `skills/attenza-intervention/SKILL.md` must all exist. Pointing Cursor at the repository root fails because those files are not there.
...
4. Open **Customize** and confirm Attenza appears with one MCP server (`attenza`) and one skill (`attenza-intervention`).
...
Enable `attenza`, complete the browser OAuth flow, and verify the tools `create_intervention`, `get_intervention`, and `cancel_intervention` appear. This proves the OAuth MCP contract. It does not load the skill, so the agent may not poll or honor expiry on its own.
