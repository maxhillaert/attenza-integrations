...
| ChatGPT / Codex | Native [`.codex-plugin`](packages/attenza-plugin/.codex-plugin/plugin.json) manifest, required registered [`.app.json`](packages/attenza-plugin/.app.json), bundled skills, and private repo marketplace | Developer-mode OAuth; complete package installed from the personal local marketplace with its skills and three MCP tools discovered in a clean session | Authenticate the local MCP client and complete an intervention round trip; use a managed workspace import for private cloud distribution |
